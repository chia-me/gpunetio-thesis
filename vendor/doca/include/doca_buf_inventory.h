 ...
 * @page doca_buf_inventory
 * @defgroup DOCA_BUF_INVENTORY DOCA Buffer Inventory
 * @ingroup DOCACore
...
/**
 * @brief Reposition the buffer to point to a new area within the mmap it was allocated with. If the new buf position is
 * not within the range of mmap it is currently allocated from then the user should instead release the buffer and
 * acquire a new one.
 *
 * @param [in] buf
 * DOCA Buf element.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_buf_inventory_buf_reuse_by_args(struct doca_buf *buf,
						  void *addr,
						  size_t len,
						  void *data,
						  size_t data_len);
 ...
 * @brief Reposition the buffer to point to a new area within the mmap it was allocated with. If the new buf position is
 * not within the range of mmap it is currently allocated from then the user should instead release the buffer and
 * acquire a new one. Addr and len will also be set so that the buffer appears full: addr == data and len == data_len
 *
 * @param [in] buf
 * DOCA Buf element.
 * @param [in] data
 ...
 * DOCA_SUCCESS - in case of success.
 */
static inline doca_error_t doca_buf_inventory_buf_reuse_by_data(struct doca_buf *buf, void *data, size_t data_len)
{
	return doca_buf_inventory_buf_reuse_by_args(buf, data, data_len, data, data_len);
}

/**
 * @brief Reposition the buffer to point to a new area within the mmap it was allocated with. If the new buf position is
 * not within the range of mmap it is currently allocated from then the user should instead release the buffer and
 * acquire a new one. Data and data_len will be reset so that the buffer appears empty: data == addr and data_len == 0
 *
 * @param [in] buf
 * DOCA Buf element.
 * @param [in] addr
 ...
 * DOCA_SUCCESS - in case of success.
 */
static inline doca_error_t doca_buf_inventory_buf_reuse_by_addr(struct doca_buf *buf, void *addr, size_t len)
{
	return doca_buf_inventory_buf_reuse_by_args(buf, addr, len, addr, 0);
}
...
/**
 * @brief Expand the inventory
 *
 * @details Add more doca_bufs to the inventory.
 *
 * @param [in] inventory
 * Inventory to expand
 * @param [in] num_elements
