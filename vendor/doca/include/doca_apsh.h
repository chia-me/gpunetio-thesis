 ...
 * @file doca_apsh.h
 * @page apsh
 * @defgroup DOCA_APSH DOCA App Shield
 *
 ...
 *
 * hold the interface information
 */
struct doca_apsh_interface;

/**
 * @brief system bash history
 *
 * hold the bash history information
 */
struct doca_apsh_bash_history;
 ...
 *
 * hold the container information
 */
struct doca_apsh_container;

/**
 * @brief system process file details
 *
 * hold the file details information
 */
struct doca_apsh_proc_file_details;
...
/**
 * @brief Set folder that system os symbol map files
 *
 *
 * @param system
 *   system handler
 * @param system_os_symbol_folder_path
 *   the folder that contains os memory map data files.
 *   this folder will be scanned in order to find the symbol map.
 *   note that this parameter is considered only if os symbol map path is not provided.
 *   the map should be created by running the doca_system_os_symbol_map tool on the system os and
 *   then placed in the os_symbol_folder.
 * @return
 *   DOCA_SUCCESS - in case of success.
 *   doca_error code - in case of failure:
 *   - DOCA_ERROR_INVALID_VALUE - if an invalid input had been received.
...
DOCA_EXPERIMENTAL
doca_error_t doca_apsh_sys_os_symbol_map_folder_set(struct doca_apsh_system *system,
...
/**
 * @brief Get array of current connections for a specified process
 *
 * This function is multi-threaded compatible with different system context,
 * meaning do not call this function simultaneously with the same system context.
 * The return array is snapshot, this is not dynamic array, need to free it.
 *
 * @note currently supported only for linux and windows 10 build systems (such as: windows 10 and windows server 2019).
 *
 * @param process
 *   Process handler
 * @param connections
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_apsh_process_netscan_get(struct doca_apsh_process *process,
					   struct doca_apsh_netscan ***connections,
					   int *connections_size);
...
/**
 * @brief Get array of all interfaces
 *
 * This function is multi-threaded compatible with different system context,
 * meaning do not call this function simultaneously with the same system context.
 * The return array is snapshot, this is not dynamic array, need to free it.
 *
 * @note currently supported only for Linux OS.
 *
 * @param system
 *   System handler
 * @param interfaces
...
DOCA_EXPERIMENTAL
doca_error_t doca_apsh_interfaces_get(struct doca_apsh_system *system,
				      struct doca_apsh_interface ***interfaces,
				      int *interfaces_size);

/**
 * @brief Destroys an interfaces data array
 *
 * @param interfaces
 *   Array of interface data opaque pointers of the system to destroy
 */
DOCA_EXPERIMENTAL
void doca_apsh_interfaces_free(struct doca_apsh_interface **interfaces);

/**
 * @brief Shadow function - get attribute value for an interface
 *
 ...
 */
DOCA_EXPERIMENTAL
const void *__doca_apsh_interface_info_get(struct doca_apsh_interface *interface, enum doca_apsh_interface_attr attr);

/**
 * @brief Get attribute value for an interface
 *
 * Get the requested info from interface handler.
 * The info is right to the snapshot (at the get function moment)
 * If the requested info is an array or string no data is copied.
 * The caller should not release the returned object.
 * full list (type and descriptions) can be found in doca_apsh_attr.h
 *
 * @param interface
 *   single interface handler
...
/**
 * @brief Get array of current containers running on the system
 *
 * This function is multithreaded compatible with different system context,
 * meaning do not call this function simultaneously with the same system context.
 * The return array is snapshot, this is not dynamic array, need to free it.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_apsh_containers_get(struct doca_apsh_system *system,
				      struct doca_apsh_container ***containers,
				      int *containers_size);

/**
 * @brief Destroys a container context
 *
 * @param containers
 *   Array of container opaque pointers of the systems to destroy
 */
DOCA_EXPERIMENTAL
void doca_apsh_containers_free(struct doca_apsh_container **containers);

/**
 * @brief Shadow function - get attribute value for a container
 *
 ...
 */
DOCA_EXPERIMENTAL
const void *__doca_apsh_container_info_get(struct doca_apsh_container *container, enum doca_apsh_container_attr attr);

/**
 * @brief Get attribute value for a container
 *
 * Get the requested info from container handler.
 * The info is right to the snapshot (at the get function moment)
 * full list (type and descriptions) can be found in doca_apsh_attr.h
 *
 * @param container
...
/**
 * @brief Get array of current processes running on the container
 *
 * This function is multithreaded compatible with different system context,
 * meaning do not call this function simultaneously with the same system context.
 * The return array is snapshot, this is not dynamic array, need to free it.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_apsh_container_processes_get(struct doca_apsh_container *container,
					       struct doca_apsh_process ***processes,
					       int *processes_size);

/**
 * @brief Get array of metadata on current process executable files and libraries
 *
 * This function is multi-threaded compatible with different system context,
 * meaning do not call this function simultaneously with the same system context.
 * The return array is snapshot, this is not dynamic array, need to free it.
 *
 * @note currently supported only for linux systems.
 *
 * @param process
 *   Process handler
 * @param files_details
 *   Array of file details opaque pointers of the process.
 *   in case process doesn't have any files, will return NULL.
 * @param files_details_size
 *   Output param, will contain size of files_details array on success.
 * @return
 *   DOCA_SUCCESS - in case of success (including the case files_details_size is zero).
 *   doca_error code - in case of failure:
 *   - DOCA_ERROR_INVALID_VALUE - if an invalid input had been received.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_apsh_proc_files_details_get(struct doca_apsh_process *process,
					      struct doca_apsh_proc_file_details ***files_details,
					      int *files_details_size);

/**
 * @brief Destroys a files_details context
 *
 * @param files_details
 *   Array of files_details opaque pointers of the process to destroy
 */
DOCA_EXPERIMENTAL
void doca_apsh_proc_files_details_free(struct doca_apsh_proc_file_details **files_details);

/**
 * @brief Shadow function - get attribute value for file details
 *
 ...
 */
DOCA_EXPERIMENTAL
const void *__doca_apsh_proc_file_details_info_get(struct doca_apsh_proc_file_details *file_details,
						   enum doca_apsh_proc_file_details_attr attr);

/**
 * @brief Get attribute value for a file
 *
 * Get the requested info from file details handler.
 * The info is right to the snapshot (at the get function moment)
 * full list (type and descriptions) can be found in doca_apsh_attr.h
 *
 * @param file_details
