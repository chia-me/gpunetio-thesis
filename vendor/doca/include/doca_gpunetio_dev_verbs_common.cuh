...
#else
#define DOCA_GPUNETIO_VERBS_ENABLE_FAST_MEMBAR 1
#endif
 ...
 */
#define DOCA_GPUNETIO_VERBS_WRITE_ONCE(x, v) (DOCA_GPUNETIO_VERBS_VOLATILE(x) = (v))
 ...
 */
__device__ static inline uint64_t doca_gpu_dev_verbs_query_globaltimer() {
    uint64_t ret;
    asm volatile("mov.u64 %0, %%globaltimer;" : "=l"(ret)::"memory");
    return ret;
}

__device__ static inline unsigned int doca_gpu_dev_verbs_get_lane_id() {
    unsigned int ret;
    asm volatile("mov.u32 %0, %%laneid;" : "=r"(ret));
    return ret;
}

__device__ static inline uint64_t doca_gpu_dev_verbs_bswap64(uint64_t x) {
    uint64_t ret;
    asm volatile(
        "{\n\t"
        ".reg .b32 mask;\n\t"
...
__device__ static inline uint32_t doca_gpu_dev_verbs_bswap32(uint32_t x) {
    uint32_t ret;
    asm volatile(
        "{\n\t"
        ".reg .b32 mask;\n\t"
...
__device__ static inline void doca_gpu_dev_verbs_store_relaxed_mmio(uint64_t *ptr, uint64_t val) {
    asm volatile("st.mmio.relaxed.sys.global.b64 [%0], %1;" : : "l"(ptr), "l"(val));
}
#endif

template <enum doca_gpu_dev_verbs_sync_scope sync_scope>
__device__ static inline void doca_gpu_dev_verbs_fence_acquire() {
...
#else
    #ifdef DOCA_GPUNETIO_VERBS_ENABLE_FAST_MEMBAR
        // fence.acquire is not available in PTX. Emulate that with ld.acquire.
        uint32_t dummy;
        uint32_t val = 0;
        if (sync_scope == DOCA_GPUNETIO_VERBS_SYNC_SCOPE_CTA)
    ...
    #else
        // fence.acquire is not available in PTX
        if (sync_scope == DOCA_GPUNETIO_VERBS_SYNC_SCOPE_CTA)
...
template <enum doca_gpu_dev_verbs_sync_scope sync_scope>
__device__ static inline void doca_gpu_dev_verbs_fence_release() {
#ifdef DOCA_GPUNETIO_VERBS_HAS_FENCE_ACQUIRE_RELEASE_PTX
    if (sync_scope == DOCA_GPUNETIO_VERBS_SYNC_SCOPE_CTA)
...
template <enum doca_gpu_dev_verbs_sync_scope sync_scope>
__device__ static inline void doca_gpu_dev_verbs_async_store_release(uint32_t *ptr, uint32_t val) {
...
template <enum doca_gpu_dev_verbs_sync_scope sync_scope>
__device__ static inline void doca_gpu_dev_verbs_async_store_release(uint64_t *ptr, uint64_t val) {
    if (sync_scope == DOCA_GPUNETIO_VERBS_SYNC_SCOPE_GPU)
...
__device__ static inline bool doca_gpu_dev_isaligned(void *ptr, size_t alignment) {
    bool status;
 ...
 */
__device__ static inline void doca_gpu_dev_verbs_memcpy_aligned_data(void *dst, void *src, size_t bytes) {
 ...
 * @param bytes - Number of bytes to copy
 */
__device__ static inline void doca_gpu_dev_verbs_memcpy_data(void *dst, void *src, size_t bytes) {
    size_t remaining_bytes = bytes;
    size_t copied_size;
    while (remaining_bytes > 0) {
        if (doca_gpu_dev_isaligned(dst, sizeof(uint64_t)) && doca_gpu_dev_isaligned(src, sizeof(uint64_t)) &&
            remaining_bytes >= sizeof(uint64_t)) {
            *(uint64_t *)dst = *(uint64_t *)src;
            copied_size = sizeof(uint64_t);
        } else if (doca_gpu_dev_isaligned(dst, sizeof(uint32_t)) &&
                   doca_gpu_dev_isaligned(src, sizeof(uint32_t)) && remaining_bytes >= sizeof(uint32_t)) {
            *(uint32_t *)dst = *(uint32_t *)src;
            copied_size = sizeof(uint32_t);
        } else if (doca_gpu_dev_isaligned(dst, sizeof(uint16_t)) &&
                   doca_gpu_dev_isaligned(src, sizeof(uint16_t)) && remaining_bytes >= sizeof(uint16_t)) {
            *(uint16_t *)dst = *(uint16_t *)src;
            copied_size = sizeof(uint16_t);
        } else {
            *(uint8_t *)dst = *(uint8_t *)src;
...
template <typename T>
__device__ static inline void doca_gpu_dev_verbs_memcpy_inl_aligned_data(T *dst, T *src, size_t bytes) {
    size_t remaining_bytes = bytes;
...
__device__ static inline T doca_gpu_dev_verbs_atomic_max(T *ptr, T val) {
    if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE) {
        ...
        *ptr = max(old_val, val);
        ...
        return ptr_aref.fetch_max(val, need_fence_acquire ? cuda::memory_order_acquire
...
template <typename T, enum doca_gpu_dev_verbs_resource_sharing_mode resource_sharing_mode>
__device__ static inline T doca_gpu_dev_verbs_atomic_add(T *ptr, T val) {
    if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE) {
        T old_val = *ptr;
        *ptr = old_val + val;
        return old_val;
...
template <typename T, enum doca_gpu_dev_verbs_resource_sharing_mode resource_sharing_mode>
__device__ static inline T doca_gpu_dev_verbs_atomic_read(T *ptr) {
    if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE)
        return *ptr;
    else
        return DOCA_GPUNETIO_VERBS_VOLATILE(*ptr);
}
...
__device__ static inline void doca_gpu_dev_verbs_lock(int *lock) {
    if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE) {
        *lock = 1;
    } else if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_CTA) {
 ...
 */
template <enum doca_gpu_dev_verbs_resource_sharing_mode resource_sharing_mode>
__device__ static inline void doca_gpu_dev_verbs_unlock(int *lock) {
    if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE) {
        *lock = 0;
    } else if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_CTA) {
...
__device__ static inline uint8_t doca_gpu_dev_verbs_load_relaxed_sys_global(uint8_t *ptr) {
    uint16_t ret;
...
__device__ static inline uint64_t doca_gpu_dev_verbs_load_relaxed_sys_global(uint64_t *ptr) {
    uint64_t ret;
...
template <enum doca_gpu_dev_verbs_resource_sharing_mode resource_sharing_mode>
__device__ static inline uint64_t doca_gpu_dev_verbs_load_relaxed(uint64_t *ptr) {
    uint64_t ret = 0;
    if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE)
        ret = *ptr;
    else if (resource_sharing_mode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_CTA)
 ...
 */
__device__ static inline uint64_t doca_gpu_dev_verbs_div_ceil_aligned_pow2(uint64_t x,
 ...
 * @return The ceiling of x / y
 */
__device__ static inline uint32_t doca_gpu_dev_verbs_div_ceil_aligned_pow2_32bits(
    uint64_t x, int denominator_shift) {
