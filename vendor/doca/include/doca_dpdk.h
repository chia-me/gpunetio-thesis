 ...
 * @file doca_dpdk.h
 * @page doca_dpdk
 * @defgroup DOCA_DPDK_BRIDGE DOCA DPDK Bridge
 * @ingroup DOCACore
 *
 ...
 *
 * @note This function should be used before calling doca_dpdk_port_probe() in case representors are required in
 * devargs.
...
/**
 * @brief Attach a DPDK port specified by DOCA device and list of representors.
 *
 * Thread unsafe API.
 *
 * It's the user responsibility to set the DPDK EAL initialization to
...
DOCA_EXPERIMENTAL
doca_error_t doca_dpdk_port_probe_with_representors(struct doca_dev *dev,
						    const char *devargs,
						    struct doca_dev_rep **reps,
						    size_t nb_reps);
...
/**
 * @brief Return the DPDK port ids associated with a DOCA device.
 * Assumption is that the doca device that was probed using doca_dpdk_port_probe().
 *
 * @param [in] dev
 * DOCA device object
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dpdk_get_port_ids(const struct doca_dev *dev,
				    uint16_t *port_ids,
				    uint16_t max_port_ids,
				    uint16_t *num_port_ids);
...
/**
 * @brief Return a new DOCA device representor associated with a DPDK port.
 * Assumption is that the DPDK port is probed using doca_dev or doca_dpdk_port_probe().
 *
 * @param [in] port_id
...
/**
 * @brief Return the VF index of a network VF representor.
 *
 * Can be used to build probe string for representors:
 * E.g., representor=vf[vf_index0, vf_index1, vf_index2]
 *
 * @param [in] rep
...
/**
 * @brief Return the SF number of a network SF representor.
 *
 * Can be used to build probe string for representors:
 * E.g., representor=sf[sf_num0, sf_num1, sf_num2]
 *
 * @param [in] rep
