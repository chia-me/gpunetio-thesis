 ...
 * @file doca_eth_rxq.h
 * @page DOCA_ETH_RXQ
 * @defgroup DOCA_ETH_RXQ DOCA ETH RXQ
 * @ingroup DOCA_ETH
 * @defgroup DOCA_ETH DOCA ETH
...
/**
 * @brief Set metadata number for doca_eth_rxq.
 * can only be called before calling doca_ctx_start().
 *
 * @note Supported for DOCA ETH RXQ instance for CPU only.
 * @note The default configuration is to disable metadata support (0 metadata per packet).
 *
 * @param [in] eth_rxq
 * Pointer to doca_eth_rxq instance.
...
/**
 * @brief Setter to enable flow tag support. User can retrieve flow tag per packet when this is enabled.
 * can only be called before calling doca_ctx_start().
 *
 * @note Supported for DOCA ETH RXQ instance for CPU only.
 * @note The default configuration is to disable flow tag support.
 *
 * @param [in] eth_rxq
 * Pointer to doca_eth_rxq instance.
 * @param [in] enable_flow_tag
...
/**
 * @brief Setter to enable RX hash support. User can retrieve RX hash per packet when this is enabled.
 * can only be called before calling doca_ctx_start().
 *
 * @note Supported for DOCA ETH RXQ instance for CPU only.
 * @note The default configuration is to disable RX hash support.
 *
 * @param [in] eth_rxq
 * Pointer to doca_eth_rxq instance.
 * @param [in] enable_rx_hash
...
/**
 * @brief Setter to enable packet headroom support. User can use doca_buf's headroom of size head_size when this is
 * enabled. can only be called before calling doca_ctx_start().
 *
 * @note Supported for DOCA ETH RXQ instance for CPU only.
 * @note Supported for DOCA_ETH_RXQ_TYPE_MANAGED_MEMPOOL mode only.
 * @note The default configuration is to disable packet headroom support.
 *
 * @param [in] eth_rxq
 * Pointer to doca_eth_rxq instance.
 * @param [in] head_size
...
/**
 * @brief Setter to enable packet tailroom support. User can use doca_buf's tailroom of size tail_size when this is
 * enabled. can only be called before calling doca_ctx_start().
 *
 * @note Supported for DOCA ETH RXQ instance for CPU only.
 * @note Supported for DOCA_ETH_RXQ_TYPE_MANAGED_MEMPOOL mode only.
 * @note The default configuration is to disable packet tailroom support.
 *
 * @param [in] eth_rxq
 * Pointer to doca_eth_rxq instance.
 * @param [in] tail_size
...
/**
 * @brief Setter to enable timestamp support. User can retrieve timestamp in nanoseconds per packet when this is
 * enabled. can only be called before calling doca_ctx_start().
 *
 * @note Supported for DOCA ETH RXQ instance for CPU only.
 * @note The default configuration is to disable timestamp support.
 * @note This feature return the real time in nanoseconds when REAL_TIME_CLOCK_ENABLE ("mlxconfig" parameter) is TRUE.
 *
 * @param [in] eth_rxq
 * Pointer to doca_eth_rxq instance.
 * @param [in] enable_timestamp
...
/**
 * @brief Set notification moderation
 *
 * @details Notification moderation affects the number of events Rx completion events.
 * Notification moderation can be modified at run time.
 *
 * @param [in] eth_rxq
 * Pointer to instance to set the moderation
 * @param [in] period_usec (0 disables moderation)
 * Period in microseconds from the first completion until event is fired
 * @param [in] comp_count (0 disables moderation)
 * Number of Rx completions until event is fired
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_set_notification_moderation(struct doca_eth_rxq *eth_rxq,
						      uint16_t period_usec,
						      uint16_t comp_count);

/**
 * @brief Apply logical queue ID for ETH's receive queue.
 * can only be called once and after calling doca_ctx_start().
 *
 * @details This logical queue ID is associated with ETH's receive queue when referring to it in DOCA Flow.
 ...
 * Pointer to doca_eth_rxq instance.
 * @param [in] queue_id
 * The logical queue ID to be used for ETH's receive queue.
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_apply_queue_id(struct doca_eth_rxq *eth_rxq, uint16_t queue_id);
 ...
 * (check doca_eth_rxq_set_metadata_num)
 *
 * @return
 * DOCA_SUCCESS - if property set successfully.
 * doca_error code - in case of failure:
...
/**
 * @brief Get the maximum packet headroom size supported by the device.
 *
 * @note In case of context of type DOCA_ETH_RXQ_TYPE_MANAGED_MEMPOOL where headroom (or tailroom) is used,
 *	 the following should be satisfied: (packet_headroom + packet_tailroom + max_packet_size) <= 8KB
 *
 * @param [in] devinfo
 * Pointer to doca_devinfo instance.
 * @param [out] max_packet_headroom
 * Maximal packet headroom size to get.
 * (check doca_eth_rxq_set_packet_headroom)
 *
 * @return
 * DOCA_SUCCESS - if property set successfully.
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_cap_get_max_packet_headroom(const struct doca_devinfo *devinfo,
						      uint16_t *max_packet_headroom);

/**
 * @brief Get the maximum packet tailroom size supported by the device.
 *
 * @note In case of context of type DOCA_ETH_RXQ_TYPE_MANAGED_MEMPOOL where headroom (or tailroom) is used,
 *	 the following should be satisfied: (packet_headroom + packet_tailroom + max_packet_size) <= 8KB
 *
 * @param [in] devinfo
 * Pointer to doca_devinfo instance.
 * @param [out] max_packet_tailroom
 * Maximal packet tailroom size to get.
 * (check doca_eth_rxq_set_packet_tailroom)
 *
 * @return
 * DOCA_SUCCESS - if property set successfully.
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_cap_get_max_packet_tailroom(const struct doca_devinfo *devinfo,
...
/**
 * @brief Get the hardware queue number of a doca_eth_rxq.
 * can only be called once and after calling doca_ctx_start().
 *
 * @details This hardware queue number is associated with ETH's receive queue.
 *
 * @param [in] eth_rxq
 * Pointer to doca_eth_rxq instance.
 * @param [out] hw_queue_num
 * The hardware queue number of the doca_eth_rxq's receive queue.
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_get_hw_queue_num(struct doca_eth_rxq *eth_rxq, uint32_t *hw_queue_num);
