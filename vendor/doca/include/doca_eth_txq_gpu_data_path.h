 ...
 * @page DOCA_ETH_TXQ_GPU_DATA_PATH
 * @defgroup DOCA_ETH_TXQ_GPU_DATA_PATH DOCA ETH TXQ GPU Data Path
...
/**
 * @brief By default, the Eth Txq completion can be checked on th CPU.
 * With this method set the Eth Txq completion on the GPU.
 * This way CUDA GPUNetIO functions can be used to check the completion of a send.
 *
 * @note Supported for DOCA ETH TXQ instance for GPU only.
 *
 * @param [in] eth_txq
...
/**
 * @brief By default, the Eth Txq UAR is on the GPU.
 * With this method set the Eth Txq UAR on the CPU.
 *
 * @note Supported for DOCA ETH TXQ instance for GPU only.
 *
 * @param [in] eth_txq
...
/**
 * @brief Set the GPU memory type for the ETH's send queue.
 * can only be called before calling doca_ctx_start().
 *
 * @note Supported for DOCA ETH TXQ instance for GPU only.
 ...
 * @param [in] eth_txq
 * Pointer to doca_eth_txq instance.
 * @param [in] sq_mem_type
 * The GPU memory type to be used for ETH's send queue.
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * doca_error code - in case of failure:
...
/**
 * Progress Txq with GPUNetIO data path via CPU proxy.
 ...
 * - DOCA_ERROR_NOT_SUPPORTED - if Txq doorbell must be rang from GPU.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_txq_gpu_cpu_proxy_progress(struct doca_eth_txq *txq);
