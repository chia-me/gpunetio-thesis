...
/**
 * @defgroup DOCA_PCC_DEVICE_SERVICES DOCA PCC Device Services
...
/**
 * @brief User service thread callback
 *
 * This is a weak symbol that the user can optionally implement in their device code.
 * If implemented, it will be called continuously by each service thread.
 *
 * @param[in] thread_id - Thread ID (0 to num_threads-1)
 *
 * @note The host application MUST call doca_pcc_set_service_thread_affinity() before
 *       doca_pcc_start() to configure the number of threads.
 */
DOCA_EXPERIMENTAL
void doca_pcc_dev_user_service_handler(uint32_t thread_id) __attribute__((weak));
