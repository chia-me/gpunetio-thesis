...
/**
 * @brief Get a bitmap of configurable device feature bits (0-63) for Virtio devices associated with the given default
 * virtio type.
 *
 * @param [in] virtio_type
...
doca_error_t doca_devemu_virtio_cap_default_type_get_configurable_device_features_63_0(
		const struct doca_devemu_virtio_type *virtio_type, uint64_t *features);

/**
 * @brief Get the maximum number of queues that can be configured for any Virtio device associated with the given
 * default Virtio type.
 *
 * @param [in] virtio_type
 * The DOCA Virtio type instance to query. Must be started.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devemu_virtio_cap_default_type_get_max_num_queues(const struct doca_devemu_virtio_type *virtio_type,
		uint16_t *num_queues);
