...
/**
 * @brief Get the maximum DPA kernel time alive supported by the DOCA device
 *
 * During the DPA process creation, the host application should read the cap and adjust the parameters of the DPA
 * threads, such as batch size and loop iterations, to ensure they remain within the specified limits.
 *
 * @param[in] devinfo - the device to query
 * @param[out] value - the maximum kernel time alive supported by the DOCA device in seconds
 *
 * @return
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dpa_cap_get_max_kernel_time_alive_supported(const struct doca_devinfo *devinfo, uint64_t *value);
...
/**
 * @brief Get DPA thread ID
 *
 * @param[in] dpa_thread - DPA thread
 * @param[out] dpa_thread_id - DPA thread ID
 *
 * @return
 * - DOCA_SUCCESS - in case of success
 * - DOCA_ERROR_INVALID_VALUE - received invalid input
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dpa_resources_create(const char *app_key,
				       const void *sprd_buf,
				       size_t sprd_buf_size,
				       struct doca_dpa_resources **resources);

/**
 * @brief Destroy a DPA resources object.
 *
 * @param[in] resources - DPA resources object to destroy
 *
 * @return
 * DOCA_SUCCESS - in case of success
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dpa_resources_destroy(struct doca_dpa_resources *resources);

/**
 * @brief Get the number of EUs in the DPA resources object.
 *
 * @param[in] resources - DPA resources object
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dpa_resources_get_num_eus(const struct doca_dpa_resources *resources, uint32_t *num_eus);

/**
 * @brief Get the EUs in the DPA resources object.
 *
 * @param[in] resources - DPA resources object
 * @param[in] num_eus - size of the EUs array allocated by the user. Must be at least the number of EUs in the DPA
 * resources object
 * @param[out] eus - array allocated by the user to be filled with EUs IDs. Must be at least the number of EUs in the
 * DPA resources object
 *
 * @return
 * DOCA_SUCCESS - in case of success
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dpa_resources_get_eus(const struct doca_dpa_resources *resources, uint32_t num_eus, uint32_t *eus);
