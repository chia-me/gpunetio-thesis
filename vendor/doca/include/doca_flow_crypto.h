 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_flow_crypto_ipsec_resource_handle(struct doca_flow_port *port,
						    uint64_t quota,
 ...
 *
 * @param [in] port
 * Port to update the sequence number state
 * @param [in] shared_res_id
 * ipsec SA shared resource ID
 * @param [in] sequence_number
 * current sequence number
 ...
 * - DOCA_ERROR_INVALID_VALUE - received invalid input.
 * - DOCA_ERROR_UNKNOWN - otherwise.
 * @note This API is used in port level mode only.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_flow_port_crypto_ipsec_update_sn(struct doca_flow_port *port,
						   uint32_t shared_res_id,
