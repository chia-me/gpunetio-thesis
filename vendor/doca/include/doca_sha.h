 ...
 * @file doca_sha.h
 * @page doca_sha
 * @defgroup DOCA_SHA DOCA SHA Engine
 ...
 * @param [in] devinfo
 * The DOCA device information.
 * @param [out] max_list_buf_num_elem
 * The maximum supported number of elements in DOCA linked-list buffer. The value 1 indicates that only a single element
 * is supported.
 *
 * @return
 * DOCA_SUCCESS - The query executed successfully, *max_list_buf_num_elem holds a valid value
 * Upon failure *max_list_buf_num_elem may be uninitialized. Typical error codes for this function include:
 *  - DOCA_ERROR_INVALID_VALUE - Invalid argument(s).
 *  - DOCA_ERROR_NOT_SUPPORTED - Device has no doca_sha support.
 * Any other error reported likely means the query failed to execute.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_sha_cap_get_max_list_buf_num_elem(struct doca_devinfo const *devinfo,
 ...
 * @param [in] devinfo
 * The DOCA device information.
 * @param [out] max_src_buf_size
 * The maximum supported buffer size in bytes.
 *
 * @return
 * DOCA_SUCCESS - The query executed successfully, *max_src_buf_size holds a valid value
 * Upon failure *max_src_buf_size may be uninitialized. Typical error codes for this function include:
 *  - DOCA_ERROR_INVALID_VALUE - Invalid argument(s).
 *  - DOCA_ERROR_NOT_SUPPORTED - Device has no doca_sha support.
 * Any other error reported likely means the query failed to execute.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_sha_cap_get_max_src_buf_size(struct doca_devinfo const *devinfo, uint64_t *max_src_buf_size);
 ...
 * @param [out] min_dst_buf_size
 * The minimum required output buffer space.
 *
 * @return
 * DOCA_SUCCESS - The query executed successfully, *min_dst_buf_size holds a valid value
 * Upon failure *min_dst_buf_size may be uninitialized. Typical error codes for this function include:
 *  - DOCA_ERROR_INVALID_VALUE - Invalid argument(s).
 *  - DOCA_ERROR_NOT_SUPPORTED - Device has no doca_sha support.
 * Any other error reported likely means the query failed to execute.
 */
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_sha_cap_get_max_num_tasks(struct doca_sha *sha, uint32_t *max_num_tasks);
 ...
 *
 * This method allocates and initializes a task. The user can choose to modify the task using the associated
 * setters while the task is not in flight. With partial tasks it is expected that a user will reuse and resubmit the
...
/**
 * Reset the partial state of this task so it can be reused. This can be called at any time. All existing partial
