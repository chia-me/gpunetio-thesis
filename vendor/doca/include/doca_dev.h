...
/**
 * @brief Check if a DOCA representor is an uplink representor.
 *
 * @note This function is only supported on Linux and non-emulated representors.
 *
 * @param [in] devinfo_rep
 * The representor of device to query.
 * @param [out] is_uplink_representor
 * 1 if representor is an uplink representor, 0 otherwise.
 * @return
 * DOCA_SUCCESS - in case of success.
 * Error code - in case of failure:
 * - DOCA_ERROR_INVALID_VALUE - received invalid input.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_is_uplink_representor(const struct doca_devinfo_rep *devinfo_rep,
							uint8_t *is_uplink_representor);
