...
Topologia (porte fisiche confermate da /etc/trex_cfg.yaml):
    ...
    dest_mac NON specificato (vedi nota sotto import) - unica configurazione
    finora confermata funzionante (2M/2M pacchetti round-trip).
...
# NOTA IMPORTANTE: NON specificare dst_mac esplicitamente. Un test diretto
# ha dimostrato che Ether() senza dst (lasciato a T-Rex) funziona (2M/2M
# pacchetti round-trip), mentre Ether(dst="ff:ff:ff:ff:ff:ff") esplicito
# NON funziona (praticamente 0 ritorno), a parita' di tutto il resto.
# Il meccanismo esatto non e' chiaro (probabile risoluzione automatica di
# T-Rex basata su trex_cfg.yaml, non un semplice broadcast statico) - ma
# la cosa verificata empiricamente e': non toccare questo campo.
